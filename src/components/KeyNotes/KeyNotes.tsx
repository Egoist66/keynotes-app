...
const KeyNotes: FC<KeyNotesProps> = memo(
  ({
    ...
    const { slides, input, message, upload, remove, triggerUpload } =
      useUploadFile();
    ...
    return (
      <>
        <div style={_sidePreviewStyle} className="keynote-main">
          <KeyNotePreview>{renderKeyNoteItem()}</KeyNotePreview>
          ...
          <div style={_keyNoteControls} id="keynote-controls">
            {enableControls ? (
              <Controls
                ...
                options={[
                  {
                    name: message || "Upload slides",
                    onClickHandler: triggerUpload,
                  },
                  {
                    name: "Remove all",
                    disabled: !slides.length ?? !data?.length,
                    onClickHandler: remove,
                  },
                //   {
                //     name: "Back",
                //     disabled: state.index === 0,
                //     onClickHandler: back(1),
                //   },
                //   {
                //     name: "Forward",
                //     disabled: !slides.length ?? !data?.length,
                //     onClickHandler: forward(1),
                //   },
                  ...
                  { name: "Fullscreen", onClickHandler: setFullScreen },
                  {
                    ...
                    disabled:
                      !slides[state.index]?.pictures ||
                      !data![state.index]!.pictures,
            ...
            {!slides.length ? null: <h3>{state.order}</h3>}
          </div>
        </div>
        ...
        <div style={{ position: "absolute", bottom: 10, right: 15 }}>
          <a
            ...
            title="download and upload into app"
            download
            href="/slides.json"
          >
            Example slides
